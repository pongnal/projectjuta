...
function ChoiceCard({
  ...
  const cardBg = isComplete
    ? "bg-[var(--jade-3)] border-[var(--jade-9)]"
    : "bg-[var(--red-3)] border-[var(--red-9)]";
  const badgeText = isComplete ? "Complete" : "Required";
  const badgeClass = isComplete
    ? "bg-[var(--jade-9)] text-white"
    ...
    <Card className={`p-4 border-2 ${cardBg}`}>
      <Flex direction="column" gap="2">
        <Flex align="center" justify="between">
          ...
          <span
            className={`rounded-full px-4 py-0.5 text-[13px] font-semibold ${badgeClass}`}
          >
            {badgeText}
          </span>
        </Flex>
...
export default function ModalMenu({
  ...
  let extra = 0;
  const sizeIndex = item.customizations?.findIndex((c: any) =>
    c.title.toLowerCase().includes("size")
  );
  if (sizeIndex !== undefined && sizeIndex !== -1) {
    const sizeVal = customValues[sizeIndex];
    const sizeOpt = item.customizations[sizeIndex].options.find(
      (o: any) => o.value === sizeVal
    );
    ...
    >
      ...
      {(item.customizations || []).map((custom: any, idx: number) => (
        <ChoiceCard
          ...
          onValueChange={(val) =>
            setCustomValues((vals: string[]) =>
              vals.map((v: string, i: number) => (i === idx ? val : v))
            )
          }
        ...
        <Card className="p-4">
          <Flex direction="column" gap="2">
            <Flex align="center" justify="between">
              ...
              <span
                className={`rounded-full px-4 py-0.5 text-[13px] font-semibold ${optionalBadgeClass}`}
              >
                Optional
              </span>
            </Flex>
      ...
      <Flex align="center" gap="3" justify="center">
        ...
        <Button variant="soft" onClick={() => setQty((q) => q + 1)}>
          +
        </Button>
      </Flex>
      <Button
        ...
        className={
          allRequiredSelected
            ? "bg-[var(--jade-9)] hover:bg-[var(--jade-10)] text-white"
            : ""
        }
